# 1120221303 俞乐楠 实验四 实验报告

## 一、程序中使用的数据结构及符号说明

| 符号              | 类型           | 说明                                              |
| --------------- | ------------ | ----------------------------------------------- |
| `Directory`     | 类            | 目录节点，包含目录名、父目录指针、子目录列表(`subdirs`)与文件列表(`files`) |
| `File`          | 类            | 文件对象，包含文件名(`name`)和内容(`content`)                |
| `root`          | `Directory*` | 根目录指针                                           |
| `current`       | `Directory*` | 当前工作目录指针                                        |
| `openFile`      | `File*`      | 当前打开的文件指针                                       |
| `lastFilename`  | `string`     | 最近保存或加载的文件名                                     |
| `D`, `END`, `F` | 文本标记         | 序列化格式中表示目录开始/结束和文件条目的标记                         |

---

## 二、模块设计及功能接口说明

### 2.1 命令解析模块（CommandParser）

* **功能**：从用户输入中解析命令和参数，分发到对应处理函数。
* **接口**：

  ```cpp
  bool parseAndExecute(const string &input);
  ```

    * **参数**：`input` 为整行命令字符串。
    * **返回**：执行结果是否继续循环。

### 2.2 目录管理模块（DirectoryManager）

* **功能**：实现目录的增删查改，包括 `mkdir`、`rmdir`、`ls`、`cd`。
* **接口**：

  ```cpp
  void makeDirectory(Directory *current, const string &name);
  void removeDirectory(Directory *current, const string &name);
  vector<string> listDirectory(const Directory *current);
  Directory* changeDirectory(Directory *current, const string &name);
  ```

### 2.3 文件管理模块（FileManager）

* **功能**：实现文件的创建、打开、写入、读取、关闭、删除。
* **接口**：

  ```cpp
  File* createFile(Directory *current, const string &filename);
  File* openFile(Directory *current, const string &filename);
  void writeFile(File *f, const string &text);
  string readFile(const File *f);
  void closeFile(File *&openFile);
  void deleteFile(Directory *current, const string &filename);
  ```

### 2.4 持久化模块（PersistenceManager）

* **功能**：将内存中的目录树和文件内容序列化保存到磁盘；从磁盘反序列化加载到内存。
* **接口**：
...
> 本实验中所谓“调度算法”即命令解析和模块分发逻辑。

1. 用户输入命令后，由**命令解析模块**识别关键字。
2. 将命令和参数传递给相应模块处理：

    * 目录操作: **DirectoryManager**
    * 文件操作: **FileManager**
    * 保存/加载: **PersistenceManager**
3. 处理完成后返回状态，决定命令循环是否继续。

**流程示意图**：

```
┌───────────────┐     ┌───────────────────┐
│ CommandParser │────>│ DirectoryManager  │
└───────────────┘     └───────────────────┘
         │                       │
         │                       ▼
         │               ┌───────────────┐
...
*代码已在 `main.cpp` 文件中完整提供，以下为核心片段及注释示例：*
...
void saveDirectory(ofstream &ofs, Directory* dir) {
    ...
    for (File* f : dir->files) {
        ofs << "F " << f->name << " " << f->content << "\n";
    }
    for (Directory* sub : dir->subdirs) saveDirectory(ofs, sub);
    ofs << "END\n";
}
```
...
`create, open, close, read, write, delete`
文件创建/删除、开/关、读/写
...
创建文件系统，打开
![img.png](image/创建.png)
再次打开后
![img.png](image/再次打开.png)


## 六、本次实验经验及体会

1. **模块化编程**：分层设计提高可读性和可维护性。
2. **树形结构理解**：深刻体会文件系统目录树的生成与遍历。
3. **持久化思路**：掌握了简单的序列化/反序列化实现。
4. **调试与测试**：通过系统测试用例完善了异常处理逻辑。
...
add_executable(Experiment4 main.cpp)
```
